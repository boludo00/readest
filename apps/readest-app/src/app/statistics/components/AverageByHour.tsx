...
const CustomTooltip: React.FC<CustomTooltipProps> = ({ active, payload }) => {
  ...
  return (
    <div className='bg-base-300 text-base-content rounded-lg px-3 py-2 shadow-lg border border-base-content/10'>
      ...
      <p className='text-lg font-bold text-primary'>{formatDuration(data.average)}</p>
      <p className='text-xs text-base-content/60'>avg per day</p>
      {data.isPeak && <p className='text-xs text-primary mt-1'>Peak hour</p>}
    </div>
  );
};
