...
const ReadingTimeline: React.FC<ReadingTimelineProps> = ({ sessions }) => {
  ...
  const isCurrentWeekNow = useMemo(() => {
    ...
    return currentWeek.weekNumber === thisWeek.weekNumber &&
           currentWeek.year === thisWeek.year;
  }, [currentWeek]);
  ...
  return (
    <div className='bg-base-200 rounded-xl p-4'>
      ...
      <div className='relative'>
        ...
        <div className='space-y-1'>
          {DAYS.map((day, dayIndex) => (
            <div key={day} className='flex items-center gap-2'>
              ...
              {/* Timeline row */}
              <div
                className='flex-1 relative h-8 bg-base-300/30 rounded overflow-hidden'
              >
                {/* Hour grid lines */}
                {HOURS.slice(1).map((hour) => (
                  <div
                    key={hour}
                    className='absolute top-0 bottom-0 w-px bg-base-content/5'
                  ...
                  .map((session) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        {width > 8 && (
                          <span className='absolute inset-0 flex items-center justify-center text-[9px] text-primary-content font-medium truncate px-1'>
        ...
        {/* Tooltip */}
        {hoveredSession && (
          <div className='absolute top-0 right-0 bg-base-300 text-base-content rounded-lg px-3 py-2 shadow-lg border border-base-content/10 text-sm z-10'>
            <div className='font-medium'>{DAYS[hoveredSession.dayIndex]}</div>
            <div className='text-base-content/70 text-xs'>
              {formatExactTime(hoveredSession.startHour)} - {formatExactTime(hoveredSession.endHour)}
            </div>
