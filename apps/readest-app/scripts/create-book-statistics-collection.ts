#!/usr/bin/env npx tsx
/**
 * Creates the 'book_statistics' collection in Appwrite database.
 *
 * Usage:
 *   npx tsx scripts/create-book-statistics-collection.ts
 *
...
async function main() {
  console.log(`\n🚀 Creating 'book_statistics' collection in Appwrite...`);
  console.log(`   Endpoint: ${endpoint}`);
  console.log(`   Project: ${projectId}`);
  console.log(`   Database: ${databaseId}\n`);

  const collectionId = 'book_statistics';
  ...
  // Create collection
  try {
    await databases.createCollection(databaseId, collectionId, 'Book Statistics', [
      'create("users")',
      'read("users")',
      'update("users")',
      'delete("users")',
  ...
  const attributes = [
    ...
    { key: 'book_hash', size: 64, required: true },
    { key: 'meta_hash', size: 64, required: false },
    { key: 'stats_json', size: 1048576, required: false },
    { key: 'created_at', size: 30, required: false },
    { key: 'updated_at', size: 30, required: false },
    { key: 'deleted_at', size: 30, required: false },
  ];
  ...
  for (const attr of attributes) {
    try {
      ...
      console.log(`  ✅ ${attr.key} (string, size=${attr.size}, required=${attr.required})`);
  ...
  const indexes = [
    ...
    { key: 'idx_user_book', type: IndexType.Key, attributes: ['user_id', 'book_hash'] },
    { key: 'idx_book_hash', type: IndexType.Key, attributes: ['book_hash'] },
    { key: 'idx_updated_at', type: IndexType.Key, attributes: ['updated_at'] },
    { key: 'idx_deleted_at', type: IndexType.Key, attributes: ['deleted_at'] },
  ];
  ...
  console.log(`\n🎉 Done! Collection 'book_statistics' is ready.`);
  console.log(
    `\n📝 Collection permissions set to: create("users"), read("users"), update("users"), delete("users")`,
  );
  console.log(`   This allows any authenticated user to manage their reading statistics.\n`);
}
